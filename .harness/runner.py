...
# Command to verify the project.
# This is a vanilla HTML/CSS/JS project with no build system.
# node --check validates JavaScript syntax; for full verification, open
# index.html in a browser manually.
VERIFY_CMD: list[str] = ["node", "--check", "app.js"]

# Path to your project's CLAUDE.md (relative to ROOT).
CLAUDE_MD_PATH: str | None = "CLAUDE.md"

# Source directories to reference in prompts.
...
def build_prompt(plan: dict, task: dict, plan_path: Path) -> str:
    ...
    return textwrap.dedent(
        ...
        5. This is a vanilla HTML/CSS/JS project — implement directly, unit tests only for complex logic.
...
def build_verify_fix_prompt(task: dict, check_output: str, context_name: str) -> str:
    ...
    return textwrap.dedent(
        ...
        {claude_md_line}
        2. Analyze the errors above — identify which are syntax or runtime failures.
        3. Fix EACH failure.
        4. Run `{verify_cmd_str}` to verify everything passes.
        5. Commit your fixes with a descriptive message.
